...
authors:
  - name: Neksha V. DeSilva
    orcid: 0000-0000-0000-0000 # UPDATE THIS
    affiliation: 1
    corresponding: true
affiliations:
...
FiaPhy addresses this need by providing a physics-based, hardware-agnostic computational library. Unlike "black box" ML approaches, FiaPhy derives air density and enthalpy dynamically from local pressure and humidity, making the system altitude-invariant—deployable from sea level to mountaintops without reconfiguration. It allows researchers in hydrology, agronomy, and embedded engineering to access radiative flux data using standard Class-0 IoT devices (8-bit microcontrollers) with minimal memory footprint (<60 bytes RAM).
